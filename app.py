...
#     """Handles image extraction and processing from Excel files"""
...
#         draw.text((100, 140), "No Image", fill="black")
...
#         """Extract images from Excel file and save them locally"""
...
# class PDFGenerator:
#     """Handles PDF generation with product catalog layout"""
...
#         line_width = len(category_name) * 8
...
#             wrap_width = 30
#         elif word_count <= 10:
#             font_size = max(8, self.config.font_size_name - 1)
#             wrap_width = 35
#         else:
#             font_size = max(7, self.config.font_size_name - 2)
#             wrap_width = 40
...
#         """Draw a single product in the grid with subtle styling"""
...
#         canvas_obj.setFillColor(colors.Color(1, 1, 1, alpha=0.7))  # Semi-transparent white
...
#             img = img.convert("RGB")
#             img = img.resize((int(self.config.image_width), int(self.config.image_height)), 
#                            Image.Resampling.LANCZOS)
            
#             # Save temporary image
#             temp_image_path = os.path.join(temp_dir, f"temp_image_{hash(str(product))}.jpg")
#             img.save(temp_image_path)
            
#             # Draw image with slight padding
#             canvas_obj.drawImage(
#                 temp_image_path,
#                 x + (self.config.cell_width - self.config.image_width) / 2,
#                 y - self.config.image_height - 15,
...
#         # Draw product name with dynamic sizing
...
#         # Draw price with consistent positioning
#         price_y = y - self.config.cell_height + 20
#         canvas_obj.setFont(self.config.font_text, self.config.font_size_price)
...
#         """Generate the complete PDF catalog with categories"""
...
#         """Generate complete catalog from Excel file"""
#         try:
...
#             if valid_products:
#                 # Calculate starting Excel row for image extraction
...
#             self.pdf_generator.create_pdf(categories, placeholder_path, output_pdf)
...
#     st.markdown("Generate professional product catalogs from Excel files with categories and images")
...
#         # Generate button
#         if st.button("🚀 Generate PDF Catalog", type="primary"):
#             try:
...
#                 st.info(f"📊 Generated catalog for {row_info}")
                
#                 # Download button
#                 st.download_button(
#                     label="📥 Download PDF Catalog",
#                     data=pdf_bytes,
#                     file_name=f"catalog_rows_{start_row}{'_to_' + str(end_row) if end_row else '_to_end'}.pdf",
...
#     font_size_name: int = 10
#     font_size_price: int = 12
#     font_size_index_title: int = 28
#     font_size_index_category: int = 14
...
#     color_text: colors.Color = colors.HexColor("#2C3E50")
#     color_price: colors.Color = colors.HexColor("#27AE60")
#     color_index_bg: colors.Color = colors.HexColor("#F8F9FA")
...
# class PDFGenerator:
#     """Handles PDF generation with product catalog layout and interactive index"""
...
#         """Draw background for index page"""
#         # Create a clean, professional background for the index
...
#         """Create an interactive index/table of contents page"""
...
#         canvas_obj.drawCentredString(self.config.page_width / 2, self.config.page_height - 50, "Product Catalog")
        
#         # Subtitle
#         canvas_obj.setFont(self.config.font_text, 16)
#         canvas_obj.setFillColor(colors.HexColor("#7F8C8D"))
#         canvas_obj.drawCentredString(self.config.page_width / 2, self.config.page_height - 75, "Table of Contents")
        
#         # Calculate starting position for categories
#         start_y = self.config.page_height - 130
#         line_height = 35
...
#             canvas_obj.drawString(self.config.margin + 20, current_y, f"{idx + 1}.")
            
#             # Draw category name as clickable link
#             canvas_obj.setFillColor(color_scheme['accent'])
#             canvas_obj.setFont(self.config.font_text, self.config.font_size_index_category)
            
#             # Create clickable link to category page
#             link_rect = (self.config.margin + 50, current_y - 5, 
#                         self.config.page_width - self.config.margin - 100, current_y + 15)
            
#             # Add bookmark link
#             canvas_obj.linkAbsolute("", f"category_{idx}", link_rect)
            
#             # Draw category name
#             canvas_obj.drawString(self.config.margin + 50, current_y, category_name)
            
#             # Draw dotted line
#             dots_start_x = self.config.margin + 50 + len(category_name) * 8 + 20
#             dots_end_x = self.config.page_width - self.config.margin - 50
            
#             canvas_obj.setStrokeColor(colors.HexColor("#BDC3C7"))
#             canvas_obj.setDash(2, 3)
#             canvas_obj.line(dots_start_x, current_y + 5, dots_end_x, current_y + 5)
#             canvas_obj.setDash()  # Reset dash
            
#             # Draw page number
#             canvas_obj.setFillColor(colors.HexColor("#7F8C8D"))
#             canvas_obj.setFont(self.config.font_text, self.config.font_size_index_category)
#             canvas_obj.drawRightString(self.config.page_width - self.config.margin - 20, current_y, str(page_number))
            
#             # Add subtle background for each category row
#             canvas_obj.setFillColor(colors.Color(1, 1, 1, alpha=0.5))
#             canvas_obj.rect(self.config.margin, current_y - 8, 
#                            self.config.page_width - 2 * self.config.margin, 25, 
#                            fill=True, stroke=False)
        
#         # Add footer note
#         footer_y = 100
#         canvas_obj.setFont(self.config.font_text, 10)
...
#     def generate_catalog(self, excel_file: str, start_row: int = 2, end_row: Optional[int] = None) -> bytes:
#         """Generate complete catalog from Excel file with interactive index"""
#         try:
#             # Load and validate data first
#             data = self.data_processor.load_excel_data(excel_file, start_row, end_row)
#             is_valid, message = self.data_processor.validate_data(data)
...
#             # Generate PDF with interactive index
#             with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as temp_pdf:
#                 output_pdf = temp_pdf.name
...
#Test 1 (best image quality and the best index )
...
#         # Handle product image with enhanced quality
#         try:
#             image_path = os.path.join("images", image_name) if image_name else placeholder_image
#             if not os.path.exists(image_path):
#                 image_path = placeholder_image
...
#             target_width = int(self.config.image_width)
#             target_height = int(self.config.image_height)
...
#             # Save temporary image with high quality
#             temp_image_path = os.path.join(temp_dir, f"temp_image_{hash(str(product))}.jpg")
#             img.save(temp_image_path, "JPEG", quality=self.config.image_quality, optimize=True, dpi=(300, 300))
            
#             # Draw image with proper centering
#             image_x = x + (self.config.cell_width - new_width) / 2
#             image_y = y - self.config.image_height - 15 + (self.config.image_height - new_height) / 2
